...
import { X, ArrowUp, ArrowRight, Building2 } from "lucide-react";
import { useNextAI } from "@/context/NextAIContext";
import { encontrarResposta, sugestoesAI, sugestoesPorContexto, type AIMessage } from "@/mocks/ai";
...
export function NextAIPanel() {
  ...
  const obraIdContexto = obraMatch ? obraMatch[1] : location.pathname === "/cronograma" ? searchParams.get("obra") : null;
  const obraContexto = obraIdContexto ? getObraById(obraIdContexto) : undefined;
  const tabContexto = obraMatch ? searchParams.get("tab") ?? "visao-geral" : location.pathname === "/cronograma" ? "cronograma" : "visao-geral";

  const sugestoes = useMemo(() => {
    if (obraContexto && sugestoesPorContexto[tabContexto]) return sugestoesPorContexto[tabContexto];
    return sugestoesAI;
  }, [obraContexto, tabContexto]);

  if (!open) return null;

  function ask(question: string) {
    setAsked(true);
    const resposta = encontrarResposta(question);
    setMessages([
      { id: "u1", autor: "usuario", texto: question },
      { id: "a1", autor: "ai", texto: resposta.texto, itens: resposta.itens, passos: resposta.passos },
    ]);
    setInput("");
  }
    ...
    <div className="fixed inset-0 z-[110] flex justify-end">
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-ink/[0.07] px-5 py-4">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 items-center justify-center rounded-[var(--radius-sm)] bg-gradient-brand-soft">
              <img src="/brand/nextline-symbol.png" alt="" className="h-5 w-5 object-contain" />
            </div>
            <div>
              <p className="text-[14.5px] font-semibold text-slate-50">Next AI</p>
              <p className="text-[11.5px] text-slate-500">
                {obraContexto ? `Contexto: ${obraContexto.nome}` : "Inteligência operacional da NextLine"}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="flex h-full flex-col justify-center gap-6">
              <div className="text-center">
                <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-brand-soft">
                  <img src="/brand/nextline-symbol.png" alt="" className="h-7 w-7 object-contain" />
                </div>
                <p className="text-[15px] font-semibold text-slate-100">Como posso ajudar hoje?</p>
                <p className="mt-1 text-[13px] text-slate-500">
                  {obraContexto
                    ? `Pergunte sobre ${obraContexto.nome} — cronograma, documentos, compras ou ocorrências.`
                    : "Pergunte sobre suas obras, pendências, fornecedores ou documentos."}
                </p>
              </div>
              <div className="space-y-2">
                {sugestoes.map((s) => (
                  <button
                    key={s}
                    onClick={() => ask(s)}
                    className="w-full rounded-[var(--radius-md)] border border-ink/10 bg-ink/[0.03] px-4 py-3 text-left text-[13px] text-slate-300 transition-colors hover:border-brand-blue/30 hover:bg-ink/[0.05] hover:text-slate-100"
                  >
                    {s}
                  </button>
                ))}
                    ...
                    <div className="max-w-[92%] space-y-3">
                      <div className="flex items-start gap-2.5">
                        <div className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-gradient-brand-soft">
                          <img src="/brand/nextline-symbol.png" alt="" className="h-3.5 w-3.5 object-contain" />
                        </div>
                      ...
                      {m.passos && (
                        <div className="space-y-1.5 pl-8">
                          {m.passos.map((p, i) => (
                            <p key={i} className="text-[12.5px] text-slate-300">
                              {p}
                            </p>
                          ))}
                        </div>
                        ...
                        <div className="space-y-2 pl-8">
                          {m.itens.map((item) => (
                            <button
                              ...
                              className="group flex w-full items-center gap-3 rounded-[var(--radius-md)] border border-ink/10 bg-ink/[0.03] px-3.5 py-3 text-left transition-colors hover:border-brand-blue/30 hover:bg-ink/[0.05]"
                            >
                              ...
                              <ArrowRight className="h-4 w-4 shrink-0 text-slate-600 transition-colors group-hover:text-brand-blue" />
                            </button>
                          ))}
                        </div>
                      )}
        ...
        <div className="border-t border-ink/[0.07] p-4">
          <div className="flex items-center gap-2 rounded-[var(--radius-md)] border border-ink/10 bg-ink/[0.04] px-3.5 py-2.5 focus-within:border-brand-blue/40">
            <input
              value={input}
              ...
              placeholder="Pergunte sobre suas obras, pendências ou documentos..."
...
export function NextAIFloatingButton() {
  ...
  return (
    ...
    >
      <img src="/brand/nextline-symbol-white.png" alt="Next AI" className="h-6 w-6 object-contain" />
    </button>
  );
}
